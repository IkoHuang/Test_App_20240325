...
dt = joblib.load("app/dt_model.joblib")
rf = joblib.load("app/rf_model.joblib")     # 雖有載入 Random Forest 模型，但不會使用，因為在網頁 Demo.(streamlit) 選擇時會出現 error message

# 左側選單 (選擇分類器)
classifier = st.sidebar.selectbox("### Select Classifier:",
                             ("KNN","LogisticRegression","SVM","DecisionTree"))
if classifier == "KNN":
    model = knn
...
s1 = st.slider("sepal length:", 3.5, 8.5, 6.0)     # 原始值：min --> 4.3，max --> 7.9，設定範圍要更大及更小一點，要能把 min 及 max 值包住，6.0 為預設起始點
s2 = st.slider("sepal width:", 1.5, 5.5, 3.5)     # 原始值：min --> 2.0，max --> 4.4，預設起始點 3.5
s3 = st.slider("petal length:", 0.6, 7.5, 4.0)     # 原始值：min --> 1.0，max --> 6.9，預設起始點 4.0
s4 = st.slider("petal width:", 0.05, 3.5, 1.8)     # 原始值：min --> 0.1，max --> 2.5，預設起始點 1.8
